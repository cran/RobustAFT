...
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"KMEDMA",1)
C
C STEP 0: INITIALIZATIONS
C ------
...
C     SET Ft(n)=FT(n-1) and Ft(1)=1.0
  ...
  410 FKMAD=RS(I)
      ...
      FMEDMAD(II,1)=FKMED
      FMEDMAD(II,2)=FKMAD
c       if (ii.eq.1) call realpr("fkmad",5,fkmad,1)
  777 CONTINUE 
      RETURN
      END
C
...
C
     ...
     + RS(N),DD(N),YY(N),SX(N,NP),SZ(N),SW(N),SIGBET(2),DUMMY(1,2)
      ...
      DATA NU/0/
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SIGSCE",1)
      NB=1
      LINT=0
...
C
   ...
   5  CONTINUE
     ...
     *     NIT,SIGMA,THETA,RS,YY,DD,SBETA,SGAMA,SX,SZ,SW,IT,EQB)           
      FA=EQB-BB
      IF (A.LE.1.E-5) CALL MESSGE(500,"BISIGA",1)
      IF (FA.GE.1.E6) THEN
        A=A*0.5
        IF (INFA.EQ.0) INFA=1
        GOTO 5
   ...
   10 CONTINUE
     ...
     *     NIT,SIGMA,THETA,RS,YY,DD,SBETA,SGAMA,SX,SZ,SW,IT,EQB)      
      ...
      IF (FB.GE.1.E6) THEN
        B=B-0.1
        IF (INFB.EQ.0) INFB=1
        IF (B.LE.0.0) CALL MESSGE(500,"BISIGA",1)
        GOTO 10
      ENDIF
   15 CONTINUE
...
C
     ...
     +          RS(N),SX(N,NP),SZ(N),SW(N),SIGBET(2),WGT(3)
      ...
      COMMON/PSIPR/IPSI,C,H1,H2,H3,XK,D
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SIGSNR",1)
      SUMI=0.D0
      BET1=SIGBET(1)
      SIGMN=SIGBET(2)
...
c      logical iver
      ...
      COMMON/PSIPR/IPSI,C,H1,H2,H3,XK,D
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SIGSGM",1)
      SUMI=0.D0
      BET1=SIGBET(1)
      SIGMN=SIGBET(2)
      IF (NIT.NE.1) GOTO 300
...
C
      G=0.D0
      IF (X.EQ.0.D0) RETURN
      IF (X.LT.0.D0.OR.P.LE.0.D0) CALL MESSGE(500,"INTGAM",1)
      CALL MACHD(6,OFLO)
      OFLO=OFLO*1.D-15
      CALL LGAMAD(P,GP)
      GIN=0.D0
...
C     IF (IWGT.EQ.2) ETA2=XK
     ...
     +       LIMIT,SUMA,ERRSTD,NEVAL,IER,WORK,IWORK)
        IF (IER.NE.0) CALL MESSGE(400+IER,"WHSKNR",0)
        LOW=-XXK
      ENDIF
      IF (LOW.LT.XXK) THEN
        HI=XXK
...
c
     ...
     +       LIMIT,SUMA,ERRSTD,NEVAL,IER,WORK,IWORK)
        IF (IER.NE.0) CALL MESSGE(400+IER,"WHSKGM",0)
        LOW=XXK
      ENDIF
      XXK=DBLE(TCU)     
      IF (LOW.LT.XXK) THEN
...
C
     ...
     + RS(N),DD(N),YY(N),SZ(N),SW(N),DUMMY(1,2)
      ...
      DATA NU/0/
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SUMRRE",1)
      NB=1
      B=0.5
      TU=1.E9
      IF (IPSI.EQ.2) TU=H3
...
C
     ...
     + RS(N),DD(N),YY(N),SZ(N),SW(N),DUMMY(1,2)
      ...
      DATA NU/0/
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SUM1RE",1)
      NB=1
      B=0.5
      TU=1.E9
      IF (IPSI.EQ.2) TU=H3
...
c 340 RES(J)=RES(J)+SUMI*DBLE(X(I,J))
  ...
  370 CONTINUE
c     DO 380 J=1,NP+1
c 380 RES(J)=RES(J)/DBLE(N)
      RES=RES/DBLE(N-NP)-0.5D0
      RETURN
      END
C
...
C
     ...
     +       RES(NP+1,NP+1)
      ...
      DATA NU/0/
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SUMRJA",1)
      NB=1
      B=0.5
      TU=1.E9
      IF (IPSI.EQ.2) TU=H3
...
C
     ...
     +          RS(N),DD(N),YY(N),SZ(N),SW(N),DUMMY(1,2)
      ...
      INTEGER IT(N),NUR0(2)
C
      IF (N.LE.0.OR.MDX.LT.N.OR.NP.LE.0) CALL MESSGE(500,"SUMXNR",1)
      NB=1
      LINT=0
